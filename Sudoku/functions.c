...
*/
bool sudoku_isValidRow(int matrix[VECTOR_SIZE][VECTOR_SIZE], int length, int row) {
...
*/
bool sudoku_isValidCol(int matrix[VECTOR_SIZE][VECTOR_SIZE], int length, int col) {
  /*Il risultato, all'inizio, è vero, poichè ancora non son state trovate
    soluzioni non valide*/
  bool result = true;
...
*/
bool sudoku_isValidSudoku(int matrix[VECTOR_SIZE][VECTOR_SIZE], int size) {
  /*Il risultato, all'inizio, è vero, poichè ancora non son state trovate
    soluzioni non valide*/
  bool result = true;
  ...
  for (int i = 0; i < size && result == true; i++) {
    result = (sudoku_isValidRow(matrix, size, i) || false);
  }
  for (int j = 0; j < size && result == true; j++) {
    result = (sudoku_isValidCol(matrix, size, j) || false);
  }
  return result;
}
...
int exec_sudoku(void) {
  ...
  // Output
  if (sudoku_isValidSudoku(sudoku, size)) {
